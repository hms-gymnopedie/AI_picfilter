...
def upgrade() -> None:
    ...
    op.create_index("idx_images_status", "images", ["status"], postgresql_where=sa.text("status != 'deleted'"))
    ...
    op.create_index("idx_styles_model_type", "styles", ["model_type"], postgresql_where=sa.text("status = 'active'"))
    ...
    op.create_table(
        ...
        sa.Column("params", postgresql.JSONB(astext_type=sa.Text()), server_default="{}", nullable=True),
    ...
    op.create_index("idx_jobs_status", "jobs", ["status"], postgresql_where=sa.text("status IN ('queued', 'processing')"))
    ...
    op.create_index("idx_comments_style_id", "comments", ["style_id", "created_at"],
